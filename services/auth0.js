...
class Auth0 {
    constructor() {
        this.auth0 = new auth0.WebAuth({
            domain: 'tech-blog-nextjs.auth0.com',
    ...
    logout = () => {
        ...
        this.auth0.logout({
            ...
            clientID: 'odZXfHCBiMY1FMx6d0YTInqwCg7y1R11'
        })
    }

    getJWKS = async () => {
        const res = await axios.get('https://tech-blog-nextjs.auth0.com/.well-known/jwks.json');
        const jwks = res.data;
        return jwks;
    }
    ...
    serverAuth = async (req) => {
        if (req.headers.cookie) {
            const token = await getCookieFromReq(req, 'jwt')
